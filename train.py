...
parser.add_argument('--network', default='FAGCANet')
...
parser.add_argument('--only_residual', action='store_false', help='regress residual rather than image')
...
parser.add_argument('--no_Dark_Channel', action='store_true')
...
if opt.network == 'FAGCANet':
    from FAGCANet import FAGCANet
    net = FAGCANet(in_c=total_inc, out_c=3, only_residual=opt.only_residual)
else:
...
for epoch in range(start_epoch, opt.epochs):
    ...
    for iter, data in enumerate(train_dataloader):
        ...
        batch_input_img, batch_input_edge,batch_input_Dark_Channel, batch_gt = data
        if len(opt.gpu_ids) > 0:
            batch_input_img, batch_input_edge,batch_input_Dark_Channel, batch_gt = batch_input_img.cuda(), batch_input_edge.cuda(), batch_input_Dark_Channel.cuda(),batch_gt.cuda()
        ...
        if opt.no_Dark_Channel:
            batch_input = batch_input_img
        else:
            batch_input = torch.cat((batch_input, batch_input_Dark_Channel), dim=1)
